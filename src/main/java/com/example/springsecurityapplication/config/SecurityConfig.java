...
//                .csrf().ignoringRequestMatchers("/authentication", "/registration")
//                .and()
...
//                .logout().logoutUrl("/logout").logoutSuccessUrl("/authentication");
        http.authorizeHttpRequests().requestMatchers("/authentication", "/registration", "/error", "/logout").permitAll().requestMatchers("/resources/**", "/static/**", "/css/**", "/js/**", "/img/**", "/product", "/product/info/{id}", "/product/search", "/admin/**").hasAnyRole("USER", "ADMIN").anyRequest().hasAnyRole("USER", "ADMIN").and().formLogin().loginPage("/authentication").loginProcessingUrl("/process_login").defaultSuccessUrl("/person account", true).failureUrl("/authentication?error").permitAll();
